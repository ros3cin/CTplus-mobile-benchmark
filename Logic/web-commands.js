...
module.exports = {
	...
	sessionData.language= lLang;
	  ...
	  if(errors != "")
		...
		if(applicationId)
			sessionData.applicationId = applicationId;
		
		if(mainActivity)
			sessionData.mainActivity = mainActivity;
	},
	stop : function(exec, adbCommandStrings, sessionData, res) {
		exec(adbCommandStrings.killApp(sessionData.applicationId), function(error, stdout, stderr) {
		    if(error != null)
		      res.send(error+"\n"+stdout+"\n"+stderr); 
		    else {
	...
	start : function(exec, adbCommandStrings, sessionData, res) {
		exec(adbCommandStrings.startApp(sessionData.currentTest, sessionData.nWarmUpIterations, sessionData.nIterations, sessionData.applicationId, sessionData.mainActivity), 
	...
	fullRun : function(exec, adbCommandStrings, sessionData, res) {
		...
		exec(adbCommandStrings.startApp(sessionData.currentTest), function(error, stdout, stderr) {
		    if(error != null)
		     res.send(error+"\n"+stdout+"\n"+stderr); 
		    else {
		      sessionData.stop=false;
